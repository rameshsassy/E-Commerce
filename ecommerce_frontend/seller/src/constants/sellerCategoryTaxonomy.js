/**
 * Seller category hierarchy (main → sub → type).
 * Keep in sync with backend data/sellerCategoryTaxonomy.js
 */
import { SELLER_MAIN_CATEGORIES } from './sellerMainCategories';
import {
...
/** Normalize curly/smart quotes so dropdown values match catalog keys. */
...
function taxonomyRichness(list) {
  if (!Array.isArray(list) || !list.length) return 0;
  return list.filter((item) => {
    const key = String(item || '').trim().toLowerCase();
    return !isOtherCategoryLabel(item) && key !== 'general';
  }).length;
}

/** Prefer the list with more real options (guards against stale API fallbacks). */
function pickRicherTaxonomyList(localList, apiList) {
  if (!apiList?.length) return localList;
  if (!localList?.length) return apiList;
  return taxonomyRichness(apiList) >= taxonomyRichness(localList) ? apiList : localList;
}

function resolveApiTaxonomyMainKey(key, apiTaxonomy) {
  const map = apiTaxonomy?.subcategoriesByMain;
  if (!map || map[key]) return key;
  const lower = key.toLowerCase();
  return Object.keys(map).find((k) => k.toLowerCase() === lower) || key;
}
...
/** Prefer API taxonomy when it is complete; otherwise local catalog (works offline / stale API). */
export function getSubcategoriesForSelection(main, apiTaxonomy) {
  const key = resolveTaxonomyMainKey(main);
  const local = getSubcategoriesForMain(key);
  const apiKey = resolveApiTaxonomyMainKey(key, apiTaxonomy);
  const fromApi = apiTaxonomy?.subcategoriesByMain?.[apiKey];
  return pickRicherTaxonomyList(local, fromApi);
}

export function getTypesForSelection(main, sub, apiTaxonomy) {
  const mainKey = resolveTaxonomyMainKey(main);
  const subKey = resolveTaxonomySubKey(mainKey, sub);
  const local = getTypesForMainSub(mainKey, subKey);
  const apiMain =
    apiTaxonomy?.typesByMainSub?.[mainKey] ||
    apiTaxonomy?.typesByMainSub?.[resolveApiTaxonomyMainKey(mainKey, apiTaxonomy)];
  const fromApi =
    apiMain?.[subKey] ||
    (subKey && apiMain
      ? Object.entries(apiMain).find(([k]) => resolveTaxonomySubKey(mainKey, k) === subKey)?.[1]
      : undefined);
  return pickRicherTaxonomyList(local, fromApi);
}
