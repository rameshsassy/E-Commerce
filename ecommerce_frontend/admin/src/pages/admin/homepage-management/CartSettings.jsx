...
const CartSettings = ({ settings, onUpdate }) => {
  ...
  useEffect(() => {
    if (settings && settings.cartIcon) {
      setEnabled(settings.cartIcon.enabled ?? true);
    }
  }, [settings]);
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      setMessage(`Cart icon has been ${newEnabled ? 'enabled' : 'disabled'}.`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl border border-glass-border">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold">Cart Icon Settings</h3>
          <p className="text-text-muted text-sm mt-0.5 animate-pulse-slow">
            Toggle visibility of the shopping cart bag icon and item counter badge.
          </p>
        </div>
        <button
          type="button"
