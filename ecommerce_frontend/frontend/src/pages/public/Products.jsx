                      ...
                      <div className="flex justify-between items-start mb-2 gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {product.compareAtPrice && product.compareAtPrice > product.price && (
                            <span className="text-sm font-bold text-text-muted line-through opacity-70">Rs. {product.compareAtPrice.toFixed(2)}</span>
                          )}
                          <span className="font-bold text-success">Rs. {product.price.toFixed(2)}</span>
                        </div>
                      </div>
                      <span className="text-xs bg-surface-hover px-2 py-1 rounded-md w-fit mb-3 text-text-muted">{product.category}</span>
                      <p className="text-sm text-text-muted mb-4 line-clamp-2 flex-1">{product.description?.replace(/<[^>]+>/g, '')}</p>
